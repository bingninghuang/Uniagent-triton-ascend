...
async def build_agent_framework(
    *,
    ...
    # Keep GatewayActor parsing opt-in for black-box agent frameworks. The
    # rollout engine may already parse model-native tool syntax (for example
    # vLLM's tool_call_parser), and enabling this second parser can make the
    # gateway parse Claude Code tool text with the wrong tool schema.
    tool_parser_name = af_cfg.get("gateway_tool_parser")
    if isinstance(tool_parser_name, str) and tool_parser_name.strip().lower() in ("", "none", "null"):
