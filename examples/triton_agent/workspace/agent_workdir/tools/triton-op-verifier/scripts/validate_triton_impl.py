...
def _missing_model_new_suggestion(tree) -> str:
    class_names = [n.name for n in ast.walk(tree) if isinstance(n, ast.ClassDef)]
    if "Model" in class_names and "ModelNew" not in class_names:
        return (
            "实现类必须命名为 ModelNew，不能是 Model。"
            "请将 `class Model` 重命名为 `class ModelNew`，并保留 forward()。"
        )
    return (
        "代码缺少 ModelNew 类或 forward 方法。"
        "实现类必须命名为 ModelNew（不能是 Model）。"
    )
...
def _check_forward_calls_kernel(result, tree, kernel_names):
    ...
    if forward_node is None:
        ...
        result["suggestion"] = _missing_model_new_suggestion(tree)
